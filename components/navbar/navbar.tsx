...
export default function Navbar({ store }) {
	...
	useEffect(() => {
		if (canvas_id && !tabs.find((tab) => tab.id === canvas_id)) {
			const canvas = Persistent.load('canvases').find((canvas) => canvas.id === canvas_id);
			const label = canvas?.label ?? 'Untitled';
			store.dispatch(actions.page({ label }));
			setTabs(tabs.concat(canvas ?? { id: String(canvas_id), label: label }));
		}
	}, [store, canvas_id, tabs]);
...
function Share() {
	...
	return (
		<>
			<button onClick={() => copyLink(setDisplay)}>SHARE</button>
