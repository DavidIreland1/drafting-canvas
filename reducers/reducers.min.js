...
/* harmony default export */ const cursor = ({
    view: function (state, props) {
        var _a = props.payload, id = _a.id, delta_x = _a.delta_x, delta_y = _a.delta_y, delta_scale = _a.delta_scale, cursor_x = _a.cursor_x, cursor_y = _a.cursor_y;
...
var Element = /** @class */ (function () {
    ...
    Element.highlight = function (element, context, cursor, highlight, line, box) {
        ...
        if (this.drawBound(element, context, cursor, highlight, line))
            action = 'stretch';
        if (this.drawRotate(element, context, cursor, box))
            action = 'rotate';
        if (this.drawResize(element, context, cursor, highlight, line, box))
            action = 'resize';
    ...
    Element.drawResize = function (element, context, cursor, color, line, box_size) {
        ...
        if (Math.abs(bounds.width) + Math.abs(bounds.height) > box_size * 4) {
            ...
            context.lineWidth = line;
            context.beginPath();
            this.boxes(element.id, bounds, box_size).forEach(function (square) { return context.rect(square.x, square.y, square.width, square.height); });
            context.fill();
            context.stroke();
        }
        context.rotate(-element.rotation);
    ...
    Element.drawRotate = function (element, context, cursor, box_size) {
        ...
        context.beginPath();
        this.boxes(element.id, bounds, box_size * 2).forEach(function (square) { return context.rect(square.x, square.y, square.width, square.height); });
        context.rotate(-element.rotation);
        context.translate(-center.x, -center.y);
        return context.isPointInPath(cursor.x, cursor.y);
    };
    ...
    Element.setFill = function (element, colors) {
        element.fill.forEach(function (fill) {
            if (fill.color === colors.from)
                fill.color = colors.to;
        });
    };
...
var Circle = /** @class */ (function (_super) {
    ...
    Circle.resize = function (ellipse, position, last_position) {
        ...
        var new_oposite = this.rotatePoint(oposite, new_center, -ellipse.rotation);
        var new_poistion = this.rotatePoint(position, new_center, -ellipse.rotation);
...
var group_read = (undefined && undefined.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
...
var Group = /** @class */ (function (_super) {
    ...
    Group.draw = function (group, context, cursor) {
        ...
        context.translate(-center.x, -center.y);
        var hovering = __spreadArray([], group_read(group.elements)).reverse().filter(function (element) { return elements_elements[element.type].draw(element, context, cursor); });
        context.translate(center.x, center.y);
        context.rotate(-group.rotation);
        context.translate(-center.x, -center.y);
    ...
    Group.getFill = function (group) {
        return group.elements.map(function (element) { return elements_elements[element.type].getFill(element); }).flat();
    };
    Group.setFill = function (group, colors) {
        group.elements.map(function (element) { return elements_elements[element.type].setFill(element, colors); });
    };
...
var Rectangle = /** @class */ (function (_super) {
    ...
    Rectangle.resize = function (rectangle, position, last_position) {
        ...
        var new_oposite = this.rotatePoint(oposite, new_center, -rectangle.rotation);
        var new_poistion = this.rotatePoint(position, new_center, -rectangle.rotation);
...
function selected(elements) {
    return flatten(elements).filter(function (element) { return element.selected; });
}
...
var interactions_read = (undefined && undefined.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
...
var interactions = {
    resize: function (state, props) {
        ...
        selected.forEach(function (element) { return elements_elements[element.type].resize(element, position, last_position); });
    },
    ...
    stretch: function (state, props) {
        var _a = props.payload, id = _a.id, position = _a.position, last_position = _a.last_position;
        var selected = state.elements.filter(function (element) { return element.selected; });
        selected.forEach(function (element) { return elements_elements[element.type].stretch(element, position, last_position); });
    },
    propertyRelative: function (state, props) {
        selected(state.elements).forEach(function (element) {
            Object.entries(props.payload).forEach(function (_a) {
                var _b = interactions_read(_a, 2), key = _b[0], value = _b[1];
    ...
    move: function (state, props) {
        ...
        selected(state.elements).forEach(function (element) { return elements_elements[element.type].move(element, position, last_position); });
    },
};
/* harmony default export */ const modifications_interactions = (interactions);
var interaction_types = Object.keys(interactions);

;// CONCATENATED MODULE: ./reducers/modifications/properties.ts
var properties_read = (undefined && undefined.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
...
/* harmony default export */ const properties = ({
    ...
    property: function (state, props) {
        selected(state.elements).forEach(function (element) {
            Object.entries(props.payload).forEach(function (_a) {
                var _b = properties_read(_a, 2), key = _b[0], value = _b[1];
                element[key] = round(value, 2);
            });
        });
    },
...
var modifications = __assign(__assign({}, modifications_interactions), properties);
/* harmony default export */ const modifications_modifications = (modifications);
var modification_types = Object.keys(modifications);

;// CONCATENATED MODULE: ./reducers/tools.ts
...
/* harmony default export */ const tools = ({
    ...
    selectOnly: function (state, props) {
        flatten(state.elements).forEach(function (element) { return (element.selected = props.payload.select.includes(element.id)); });
    },
